...
- **Dev servers**: `just dev` (parallel: API + workers + frontend). Or individually: `just backend dev`, `just ui dev`.
...
Backend check commands run inside `src/` (via `just backend lint`, etc.). Frontend commands run inside `ui/` (via `just ui lint`, etc.).
...
- Frontend unit: `pnpm run test:unit` (run via `just ui unit-test`)
