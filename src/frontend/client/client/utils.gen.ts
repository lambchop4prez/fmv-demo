...
function checkForExistence(options: Pick<RequestOptions, 'auth' | 'query'> & {
  headers: Headers
}, name?: string): boolean {
  if (!name) {
    return false;
...
function headersEntries(headers: Headers): Array<[string, string]> {
...
export function createInterceptors<Req, Res, Err, Options>(): Middleware<
  Req,
  Res,
  Err,
  Options
